...
namespace Test_Average
{
    public partial class Form1 : Form
    {
        ...
        // Average 方法接受一個 double 陣列參數，及實際讀入的 count
        // 並回傳該陣列前 count 個值的平均值（四捨五入到小數點第一位）。
        private double Average(double[] scores, int count)
        {
        ...
        private double Highest(double[] scores, int count)
        {
            if (scores == null || count <= 0)
            {
                return 0.0;
        ...
        private double Lowest(double[] scores, int count)
        {
            if (scores == null || count <= 0)
            {
                return 0.0;
